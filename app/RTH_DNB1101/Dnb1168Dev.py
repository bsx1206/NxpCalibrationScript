...
class RREG(Enum):
    ...
    VoltGuard     = 0xFFE2001
    VoltMain      = 0xFFE2000    
    HwChksumMTP   = 0xFFE200A
    SrvReqData    = 0xFFD0012

class ChainDev(object):
    ...
    def Close_Board(self, name = "ChainDev", port = "COM1", target_ic_num = 1):
        self._name = name
        self._port = port
        self._phy = phy.ser(port)
        self._obj = api.chain(self._phy)
    ...
    def Initialize(self):
        ...
        #self._phy.tx('sdc 0\n',delay=0.1)
        self._phy.tx("RT FF20000 2\n")                #解锁MTPLOCK (FF全广播 02寄存器 0000)
        time.sleep(0.1)
        self._phy.tx("RT FFF007D 2\n")                #切Bank 到7D
        time.sleep(0.1)
        self._phy.tx("RT FF2FFFF 2\n")                #7D[2] Gate service requests FFFF:所有的
        time.sleep(0.1)
        self._phy.tx("RT FFF0000 4\n")                #切回Bank0
        time.sleep(0.1)
        self._phy.tx("RT FFCFFE3 4\n")                  #获取状态：SrvReqData
        time.sleep(0.1)
        self._phy.tx("RT FFD0012 1\n")                  #获取状态：SrvReqData
        time.sleep(0.1)
        self._phy.tx("RT FFAFFFF 4\n")                #设置SetSrvReqMask ：FFFF
        time.sleep(0.1)
        self._phy.tx("RT FFB0004 4\n")                #设置工作模式：Normal
        time.sleep(0.1)
        self._phy.tx("sdc 1 2\n")                     #send a dummy command. Default on
        time.sleep(0.1)
        self._enum_ic_num = 0
        return True
    ...
    def Enumerate(self):
        ...
        for retry_times in range(0, retry_count):
            LOG.INF(s="%s try enumerating... %d/%d" % (self._name, retry_times + 1, retry_count))
            self._obj.en()
    ...
    def GetMTP(self):
        self._MTP = self._obj.get_MTP()
        return self._MTP
        
    def __ReadRegister(self, reg:RREG, repeat = 1):
        assert reg in RREG
        fmt_reg = f'{reg.value:07X}'
        retry_count = 5
        for retry_times in range(0, retry_count):
            DTS = self._obj.cmd2(fmt_reg, repeat)
            if None == DTS: continue
    ...
    def ReadRegister(self, reg:RREG, repeat = 1):
        r = self.__ReadRegister(reg, repeat)
        if reg.name[:4] == 'Temp':
            if isinstance(r, list):
                return list(map(api.cal_temperature, r))
            else:
                return api.cal_temperature(r)
        elif reg.name[:4] == 'Volt':
            if isinstance(r, list):
                return list(map(api.cal_voltage, r))
            else:
                return api.cal_voltage(r)
        else:
            return r
            assert False
    ...
    def StartBalanceOne(self, chan:int, current_ma:float, delay_sec:int = sys.maxsize):
        current_bias = 5.0 # mA
        current_lsb = 13.3 # mA
        ...
        cmd_raw = f"RT {chan:02X}80000 2\n"
        print(cmd_raw)
        self._phy.tx("RT FF9400 2\n")   #电压低压1.2V
        self._phy.tx("RT FF6000 2\n")  #内部电流源
        self._phy.tx(cmd_raw)         #SetbalVolt
        code = 0x81000 | current_raw | delay_raw
        cmd_raw = f"RT {chan:02X}{code:05X} 2\n"
        print(cmd_raw)
        self._phy.tx(cmd_raw)
        rx_buf = self._phy.rx()
        print(rx_buf)
    ...
    def GetRth(self, ic_id):
        self._phy.tx("RT FF20000 2\n")
        time.sleep(0.5)
        self._phy.tx("RT FFF017E 2\n")
        time.sleep(0.5)
        #Rth_values_w= 0
        for ID in ic_id:
            if  int(ID)<16:
                ...
                ID = hex(int(ID))
                ID = str(ID[2:4])
            command = "RT " + str(ID) + "80000" + " 2\n"
            #print(command)
            self._phy.tx(command)
            temp = self._phy.rx(command)
            print (temp)
            Rth_values_w=Rth_values_w+1
            time.sleep(0.5)
        self._phy.tx("RT FFF0000 2\n")
